...
public class AuditQueryTests : IDisposable
{
    ...
    public async Task ListAuditEvents_ByResourceId_ReturnsFormLifecycleEvents()
    {
        ...
        using HttpResponseMessage actorEventsResponse = await Client.GetAsync(
            "/api/audit/events?actorId=auditor-1&resourceType=form-definition&limit=50");
        Assert.Equal(HttpStatusCode.OK, actorEventsResponse.StatusCode);

        List<AuditEventDto> actorEvents =
            (await actorEventsResponse.Content.ReadFromJsonAsync<List<AuditEventDto>>())!;
        ...
        using HttpResponseMessage auditResponse = await Client.GetAsync(
            $"/api/audit/events?resourceType=form-definition&resourceId={createdEvent.ResourceId}");
        Assert.Equal(HttpStatusCode.OK, auditResponse.StatusCode);

        List<AuditEventDto> events = (await auditResponse.Content.ReadFromJsonAsync<List<AuditEventDto>>())!;
    ...
    {
        FormResponseDto response = await CreatePublishedResponseAsync("audit-response");

        using HttpResponseMessage deleteResponse = await Client.DeleteAsync(
            $"/api/responses/{response.Id}?reason=Entered%20in%20error");
        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

        using HttpResponseMessage auditResponse = await Client.GetAsync(
            $"/api/audit/events?resourceType=form-response&resourceId={response.Id}");
        Assert.Equal(HttpStatusCode.OK, auditResponse.StatusCode);
    ...
    {
        await CreatePublishedResponseAsync("actor-query");

        using HttpResponseMessage auditResponse = await Client.GetAsync("/api/audit/events?actorId=auditor-1");
        Assert.Equal(HttpStatusCode.OK, auditResponse.StatusCode);
